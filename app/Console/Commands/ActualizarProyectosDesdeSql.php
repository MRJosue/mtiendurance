...
class ActualizarProyectosDesdeSql extends Command
{
    protected $signature = 'actualizar:proyectos-desde-sql
                            {--adminId=1 : ID de usuario fallback si no existe client_id en users}
                            {--sql=database/seeders/SQL/LastProyect.sql : Ruta del SQL a importar}
                            {--limit=5000 : Cuantos registros tomar de project (orderByDesc + limit + reorden asc)}
                            {--dry-run : Solo simula, no inserta ni ejecuta SQL}
                            {--hard : Trunca también tablas dependientes (chats, mensajes_chat, archivos_proyecto) para evitar FK}';

    protected $description = 'Reemplaza tabla legacy project desde SQL y reconstruye proyectos (project -> proyectos).';
    ...
    public function handle()
    {
        ...
        $sqlRelPath = (string) $this->option('sql');
        $sqlPath    = base_path($sqlRelPath);

        $limitOpt = $this->option('limit');
        $limit    = is_numeric($limitOpt) ? (int)$limitOpt : 0;

        $this->info('===> Paso 1: Reemplazar tabla legacy `project` cargando SQL');
        $this->reemplazarTablaProjectDesdeSql($sqlPath, $dry);

        $this->info('===> Paso 2: Vaciar `proyectos` y reconstruir desde `project`');
        $this->reconstruirProyectosDesdeProject($adminId, $limit, $dry, $hard);

        $this->info('✔ Actualización finalizada ' . ($dry ? '(DRY-RUN)' : ''));
        return self::SUCCESS;
    }

    /* =========================================================
     * PASO 1) Reemplazar tabla project desde SQL
     * ========================================================= */
    protected function reemplazarTablaProjectDesdeSql(string $sqlPath, bool $dry): void
    {
        if (!File::exists($sqlPath)) {
            $this->error("No existe el archivo SQL: {$sqlPath}");
            return;
        }

        $sql = File::get($sqlPath);

        // Tip: algunos dumps traen DELIMITER, PROCEDURE, triggers etc.
        // DB::unprepared suele ser lo más compatible para dumps “planos”.
        if ($dry) {
            $this->line("[DRY] Truncaría `project` y ejecutaría SQL desde: {$sqlPath}");
            return;
        }

        DB::statement('SET FOREIGN_KEY_CHECKS=0');

        // Si el SQL ya trae DROP/CREATE/TRUNCATE, esto no estorba.
        // Si tu dump trae INSERTs únicamente, esto asegura “reemplazo total”.
        try {
            if (Schema::hasTable('project')) {
                DB::table('project')->truncate();
                $this->line(" + Truncada tabla `project`");
            }
        } catch (\Throwable $e) {
            // Si truncate falla por FK o engine, intentamos DELETE.
            $this->warn(" ! truncate(project) falló, intentando DELETE: {$e->getMessage()}");
            DB::statement('DELETE FROM `project`');
        }

        try {
            DB::unprepared($sql);
            $this->line(" + SQL importado correctamente a legacy (`project`) desde {$sqlPath}");
        } catch (\Throwable $e) {
    ...
    /* =========================================================
     * PASO 2) Vaciar proyectos y reconstruir
     * ========================================================= */
    protected function reconstruirProyectosDesdeProject(int $fallbackUserId, int $limit, bool $dry, bool $hard): void
    {
        DB::statement('SET FOREIGN_KEY_CHECKS=0');

        if ($dry) {
            $this->line("[DRY] Vaciaría `proyectos`" . ($hard ? " + dependencias" : "") . " y reconstruiría desde `project`");
        } else {
            $this->vaciarTablasProyectos($hard);
        }

        $this->migrarProjectsAProyectos($fallbackUserId, $limit, $dry);
    ...
    /**
     * Vacía `proyectos` y, si --hard, también algunas tablas dependientes típicas.
     * Ajusta aquí según tus FKs reales.
     */
    protected function vaciarTablasProyectos(bool $hard): void
    {
        // OJO: Si tienes FKs desde otras tablas (pedidos, tareas, etc.)
        // y NO quieres truncarlas, entonces no uses --hard y
        // considera borrar por proyecto_id con lógica específica.
        if ($hard) {
            foreach (['mensajes_chat', 'chats', 'archivos_proyecto'] as $tbl) {
                if (Schema::hasTable($tbl)) {
                    DB::table($tbl)->truncate();
                    $this->line(" + Truncada `{$tbl}`");
                }
            }
        }
        ...
        if (Schema::hasTable('proyectos')) {
            ...
            $this->line(" + Truncada `proyectos`");
        }
    }

    /* =========================================================
     * MIGRACIÓN project -> proyectos (basada en tu ejemplo)
     * ========================================================= */
    protected function migrarProjectsAProyectos(int $fallbackUserId, int $limit, bool $dry): void
    {
        $q = DB::table('project')
        ...
        $this->line(" = Se procesarán " . $projects->count() . " registros desde `project`");
        ...
        foreach ($projects as $p) {
            ...
            if ($dry) {
                $this->line(" + [DRY] Insertaría proyectos.id={$row['id']} usuario_id={$row['usuario_id']}");
                continue;
            }
    ...
    /* ============ REGLA: resolver nombre del archivo (igual que tu ejemplo) ============ */
    ...
    protected function insertArchivoDisenoDesdeLegacy(int $proyectoId, int $usuarioId, string $nombreLegacy): void
    {
        ...
        if ($nombreLegacy === '' || $nombreLegacy === '0') return;
        ...
        if ($yaExiste) {
            ...
            $base    = Str::limit(Str::slug($base, '_'), 80, '');
    ...
    /* ================== helpers (igual estilo que tu ejemplo) ================== */

    protected function ajustarAutoIncrement(string $table, string $pk, bool $dry): void
    {
    ...
    {
        $this->logFilePath = storage_path('logs/actualizar_proyectos_desde_sql.log');
        $dir = dirname($this->logFilePath);
        if (!File::exists($dir)) File::makeDirectory($dir, 0755, true);

        $header  = "==== ACTUALIZAR PROYECTOS DESDE SQL ====\n";
