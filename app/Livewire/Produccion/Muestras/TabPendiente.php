...
class TabPendiente extends Component
{
    use WithPagination;

    public array $selected = [];
    public string $estadoColumna = 'SOLICITADA'; // para la columna "Solicitado por"
    ...
    public string $f_estatus = ''; // opcional, por si quieres ver otros además de PENDIENTE
    ...
    public function marcarSolicitada(array $ids = []): void
    {
        ...
        Pedido::deMuestra()->whereIn('id', $ids)
            ->update(['estatus_muestra' => 'SOLICITADA']);
    ...
    public function getPedidosProperty()
    {
        ...
        // Solicitado por (usuario del último estado $estadoColumna, ej. SOLICITADA)
        if ($this->f_usuario !== '') {
            $t = "%{$this->f_usuario}%";
        ...
        // Estatus extra (por si quieres ver más de PENDIENTE)
        if ($this->f_estatus !== '') {
    ...
    public function render()
    {
        ...
        return view('livewire.produccion.muestras.tab-pendiente', [
            'pedidos'          => $pedidos,
