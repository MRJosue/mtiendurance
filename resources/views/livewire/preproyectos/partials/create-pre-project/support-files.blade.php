<div class="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <label class="block text-sm font-semibold text-slate-800 dark:text-slate-100">Archivos de apoyo</label>
            <p class="text-xs text-slate-500 dark:text-slate-400">Sube referencias, artes, documentos o paquetes comprimidos.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div>
            <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Seleccionar archivos</label>
            <input
                type="file"
                wire:model="files"
                multiple
        ...
        <div class="flex items-center gap-3">
            ...
            <span wire:loading wire:target="procesarArchivos" class="inline-flex items-center text-sm text-blue-600 dark:text-blue-400">
                Subiendo…
    ...
    @if ($uploadedFiles)
        <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-800/70">
            <div class="mb-3 flex items-center justify-between gap-3">
                <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100">Vista Previa de Archivos</h3>
                <span class="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:ring-slate-700">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                @foreach ($uploadedFiles as $file)
                    <div class="flex items-center gap-3 rounded-xl bg-white p-3 ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
                        ...
                        @else
                            <div class="flex h-14 w-14 items-center justify-center rounded-lg bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            @if (!$file['preview'])
                                <a href="{{ $file['preview'] }}" target="_blank" class="text-xs font-medium text-blue-600 hover:underline dark:text-blue-400">
                                    Abrir archivo
                                </a>
                            @endif
                        </div>
